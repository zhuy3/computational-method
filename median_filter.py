...
def get_median(data):
      ...
      if size % 2 == 0: # length is an even number
            median = (data[size//2]+data[size//2-1])/2
            data[0] = median
      if size % 2 == 1: # length is an add number
            median = data[(size-1)//2]
      data[0] = median
...
def median_replece(pos_list,data_steam,filter_len = 7):
      ...
      for pos_s in pos_list:#pos_s : adjacent group of noise
            ...
            for pos in pos_s:#pos : location of each noise
                  pos_start = int(pos-diff)#len is 5 as an example--start three positions before pos
                  pos_end = int(pos+diff)#end three positions after pos
...
def median_filter(clean_wav,degraded_wav,detect_wave,out_wave):
      ...
      bk_inf= [[]]#devide click positions into adjacent groups
      for i in range(len(bk_positions)-1):
            bk_inf[-1].append(bk_positions[i])
            #determine if there is a missing data
